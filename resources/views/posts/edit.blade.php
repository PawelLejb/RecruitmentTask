...
<body>
@yield('header')
{!! Form::model($post,['action' => ['App\Http\Controllers\PostsController@update',$post],'method'=>'PUT']) !!}
@if ($errors->any())
...
<div class="form-group">
    ...
    <select name="categories[]" id="categories" class="form-control" multiple="multiple">
        @foreach($cat as $key => $category)
            ...
            <option value="{{ $key }}"{{(in_array( $category,$selectCatList)) ? 'selected="selected"':'' }}>{{$category}}</option>
        @endforeach
